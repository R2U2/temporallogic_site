...
void printFormula(size_t formula_length) {
	...
	if (visited_right == 1) {
		...
		case '-' :
		case '>' :
		  /*OLD: sprintf(formula, "%s -> ", formula, current->me);*/
		  sprintf(formula, "%s -> ", formula); /*2021: fix?*/
		  break;
		default :
...
int printSubformulaVar_makeGraph(struct node *local_root) {
	...
	switch (local_root->me[0]) {
		...
		if (tgba) {
		    ...
		    if (smvFlag == 's') { /*SAL*/
			fprintf(SOUT, "       "); /*indent SAL module*/
			/*OLD: fprintf(SOUT, "S%s = ", subformula, temp, subformula);*/
			fprintf(SOUT, "S%s = ", subformula); /*KYR: 2021 fix?*/
		    } /*end if*/
		    else { /*Cadence SMV or NuSMV*/
		      /*OLD: fprintf(SOUT, "S%s := ", subformula, temp, subformula);*/
		      fprintf(SOUT, "S%s := ", subformula);  /*KYR: 2021 fix?*/
...
int main(int argc, char **argv) {
    ...
    char tableauFile[20];        /*name of the smv input file representing the symbolic automaton*/
    char tableauVarFile[20];     /*name of the smv variable-order input file for the symbolic automaton*/
    char temp[75];               /*NOTE: this needs to be bigger than the file name strings (above): x/t/fout + tableauFile + s/t/fout*/
    ...
    /*SMV model variables*/
    /*FILE *OUT;             /*SMV (tableau.smv) output file*/
	...
	switch (flagBuf[1]) {
		...
		else if (strlen(flagBuf) == 2) { /*SAL-style output*/
		    smvFlag = 's';
		} /*end if*/
		else {
	...
	if (smvFlag == 'c') { /*CadenceSMV-style comments*/
	    ...
	    fprintf(stderr, "ERROR: Unrecognized SMV Flag: %c", smvFlag);
	    exit(1); /*die*/
	} /*end else*/
    } /*end else*/
    fprintf(OUT, "formula: ");
	...
	switch(varOrder) {
		...
		break;
	    default : 
		fprintf(stderr, "ERROR: Unrecognized variable order \'%c\'\n", varOrder);
		exit(1);
	} /*end switch*/
